...
def heuristic(board, cards, banners, player):
    h = 0
    h += sum(banners[player])

    # could also give points for having a banner that belongs to a card that is locked into place
    # so get banner at a position and if there are none of that card in the board, give points to the
    # player that has that banner
    pBanners = banners[player]
    for i in range(len(pBanners)):
        if pBanners[i] == 1:  # if the player has a banner of that color
            color = i + 2
            if(color not in board):
                h += 1  # give points to player that has that banner since it is locked into place
...
def minimax(board, cards, banners, player):
    ...
    value = minvalue(board, cards, banners, player,
                     best, -math.inf, math.inf, count)
    for move in moves[1:]:
        v = minvalue(board, cards, banners, player,
                     move, -math.inf, math.inf, count)
        if v > value:
            best = move
            value = v
...
def minvalue(board, cards, banners, player, move, alpha, beta, count):
    ...
    # if count reaches a certain point call a heuristic function
    # to determine who has more banners and return a value based on that I think
    if(count == 10):
        pass
...
def maxvalue(board, cards, banners, player, move, alpha, beta, count):
